...
public class Dialog4Batch extends PopupWindow implements OnClickListener {
    ...
    public Dialog4Batch(Activity context, Integer price) {
        super(context);
        ...
        mMenuView = inflater.inflate(R.layout.dialog_select_batch, null);
        initView(mMenuView);
    ...
    private void selectPrice(int price) {
        ...
        if (mPrice < minPrice) {
            myTool.showInfo("批量生产不低于 " + minPrice + " ！");
            mPrice = minPrice;
            return;
        }

        if (mPrice > maxPrice) {
            myTool.showInfo("批量生产不超过 " + maxPrice + " ！");
            mPrice = maxPrice;
            return;
        }
